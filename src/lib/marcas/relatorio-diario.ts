...
function intervaloUtcDoDiaBrasilia(dataIso: string): { inicio: string; fim: string } {
  // Brasilia = UTC-3. Mantemos o intervalo explícito para o relatório diário.
...
function emailsAdicionais(): string[] {
  return (process.env.RELATORIO_MARCAS_EMAILS ?? '')
    .split(/[;,]/)
...
async function destinatariosProprietarios(): Promise<string[]> {
  ...
  const emails = usuariosAuth.users
    .filter((usuario) => ids.has(usuario.id) && usuario.email)
...
async function nomesDosExecutivos(ids: string[]): Promise<Map<string, string>> {
  ...
  for (const item of data ?? []) {
    mapa.set(item.usuario_id as string, String(item.nome ?? 'Executivo'))
  }
  return mapa
}
...
function montarHtml(dataReferencia: string, marcas: MarcaDoRelatorio[]): string {
  ...
  const linhas = marcas.map((item) => `
    <tr>
      <td style="padding:12px 10px;border-bottom:1px solid #eee;font-weight:700;color:#161616">${escapar(item.marca)}</td>
      <td style="padding:12px 10px;border-bottom:1px solid #eee;color:#444">${escapar(item.anunciante)}</td>
      <td style="padding:12px 10px;border-bottom:1px solid #eee;color:#444">${escapar(item.executivo)}</td>
      <td style="padding:12px 10px;border-bottom:1px solid #eee;color:#666;white-space:nowrap">${escapar(formatarDataHora(item.criadoEm))}</td>
      <td style="padding:12px 10px;border-bottom:1px solid #eee;color:${item.revisaoStatus === 'pendente' ? '#8A5700' : '#087A55'};font-weight:700">${item.revisaoStatus === 'pendente' ? 'Pendente' : 'Revisada'}</td>
    </tr>`).join('')

  const urlBase = (process.env.NEXT_PUBLIC_APP_URL ?? '').replace(/\/$/, '')
  const botao = urlBase
    ? `<a href="${escapar(urlBase)}/configuracoes/marcas" style="display:inline-block;background:linear-gradient(90deg,#F72585,#6C4CFF);color:white;text-decoration:none;font-weight:700;padding:12px 20px;border-radius:10px">Revisar marcas</a>`
    : ''

  const conteudo = marcas.length === 0
    ? '<div style="padding:22px;background:#F7F5FA;border-radius:12px;color:#555">Nenhuma nova marca foi cadastrada por executivos neste período.</div>'
    : `<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr style="background:#F7F5FA;text-align:left"><th style="padding:10px">Marca</th><th style="padding:10px">Anunciante</th><th style="padding:10px">Cadastrada por</th><th style="padding:10px">Data</th><th style="padding:10px">Revisão</th></tr></thead><tbody>${linhas}</tbody></table></div>`

  return `<!doctype html><html><body style="margin:0;background:#F2F0F6;font-family:Arial,sans-serif;color:#161616"><div style="max-width:900px;margin:0 auto;padding:28px 18px"><div style="background:linear-gradient(120deg,#F72585,#7B2FF7);padding:24px;border-radius:18px 18px 0 0;color:white"><div style="font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;opacity:.85">Chatbot 2.0 · Governança</div><h1 style="margin:8px 0 0;font-size:25px">Novas marcas cadastradas</h1><p style="margin:8px 0 0;opacity:.9">Relatório de ${formatarData(dataReferencia)}</p></div><div style="background:white;padding:24px;border-radius:0 0 18px 18px"><div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:20px"><div style="background:#F7F5FA;border-radius:12px;padding:14px 18px"><div style="font-size:11px;color:#777;text-transform:uppercase;font-weight:700">Novas marcas</div><div style="font-size:24px;font-weight:800;margin-top:3px">${marcas.length}</div></div><div style="background:#FFF8EC;border-radius:12px;padding:14px 18px"><div style="font-size:11px;color:#8A5700;text-transform:uppercase;font-weight:700">Pendentes de revisão</div><div style="font-size:24px;font-weight:800;margin-top:3px;color:#8A5700">${pendentes}</div></div></div>${conteudo}${botao ? `<div style="margin-top:22px">${botao}</div>` : ''}<p style="margin:24px 0 0;font-size:11px;line-height:1.5;color:#888">Marcas cadastradas na Nova Consulta ficam disponíveis imediatamente para não interromper a jornada comercial. Revise apenas grafia e associação ao anunciante quando necessário.</p></div></div></body></html>`
}

export async function enviarRelatorioDiarioDeMarcas(referencia = new Date()): Promise<{
  ...
  const { data, error } = await supabase
    ...
    .select('marca_id, cliente_id, criado_por, criado_em, revisao_status, marca:marcas(nome), cliente:clientes(nome)')
    .eq('origem', 'consulta')
  ...
  const marcas: MarcaDoRelatorio[] = linhas.map((linha) => ({
    marca: linha.marca?.nome ?? 'Marca',
    anunciante: linha.cliente?.nome ?? 'Anunciante',
    executivo: linha.criado_por ? nomes.get(linha.criado_por) ?? 'Executivo' : 'Executivo',
    criadoEm: linha.criado_em,
    revisaoStatus: linha.revisao_status,
  }))

  const destinatarios = await destinatariosProprietarios()
  await enviarEmailMicrosoft({
    para: destinatarios.map((email) => ({ email })),
    assunto: `[Chatbot 2.0] Novas marcas · ${formatarData(dataReferencia)} · ${marcas.length}`,
    html: montarHtml(dataReferencia, marcas),
  })

  const { error: erroRegistro } = await supabase
    .from('relatorio_marcas_diario')
    .insert({ data_referencia: dataReferencia, quantidade: marcas.length })
