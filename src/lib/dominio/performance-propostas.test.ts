...
function linha(parcial: Partial<LinhaParaPerformance> = {}): LinhaParaPerformance {
  return {
    ...
    aprovacao_status: parcial.aprovacao_status ?? 'nao_requerida',
    criado_em: parcial.criado_em,
  }
}

function linhaExecutivo(parcial: Partial<LinhaExecutivoPerformance> = {}): LinhaExecutivoPerformance {
  return { ...linha(parcial), usuario_id: parcial.usuario_id ?? 'u-1', executivo_nome: parcial.executivo_nome ?? 'Executivo 1' }
}
...
describe('selecionarVersoesAtuais', () => {
  it('mantém apenas a versão mais recente da mesma família', () => {
    ...
    expect(atuais).toHaveLength(1); expect(atuais[0].id).toBe('v2'); expect(atuais[0].valor_total_comercial).toBe(150)
  })
  ...
  it('não conta proposta pendente ou rejeitada como ofertada', () => {
    expect(selecionarVersoesAtuais([
      linha({ id: 'pendente', grupo_versao_id: 'a', aprovacao_status: 'pendente' }),
      linha({ id: 'rejeitada', grupo_versao_id: 'b', aprovacao_status: 'rejeitada' }),
    ])).toEqual([])
  })

  it('mantém a versão aprovada anterior enquanto uma revisão aguarda aprovação', () => {
    const atuais = selecionarVersoesAtuais([
      linha({ id: 'v1', grupo_versao_id: 'g', versao: 1, aprovacao_status: 'aprovada' }),
      linha({ id: 'v2', grupo_versao_id: 'g', versao: 2, aprovacao_status: 'pendente', valor_total_comercial: 200 }),
    ])
    expect(atuais.map((item) => item.id)).toEqual(['v1'])
  })
})
...
describe('calcularMetricasPerformance', () => {
  it('calcula conversão apenas sobre negociações decididas', () => {
    ...
    expect(metricas.propostas).toBe(4); expect(metricas.fechadas).toBe(1); expect(metricas.perdidas).toBe(1); expect(metricas.emNegociacao).toBe(2); expect(metricas.conversao).toBe(50); expect(metricas.valorFechado).toBe(90)
  })
  it('não inventa conversão quando nenhuma negociação foi decidida', () => { expect(calcularMetricasPerformance([linha({ grupo_versao_id: '1' })]).conversao).toBeNull() })
})

describe('calcularRankingExecutivos', () => {
  it('ordena por vendido e depois por ofertado', () => {
    const ranking = calcularRankingExecutivos([
      linhaExecutivo({ usuario_id: 'u-a', executivo_nome: 'Ana', grupo_versao_id: 'a1', valor_total_comercial: 500 }),
    ...
    expect(ranking.map((item) => item.nome)).toEqual(['Carla', 'Bruno', 'Ana']); expect(ranking[0].metricas.valorFechado).toBe(200); expect(ranking[0].metricas.valorProposto).toBe(450)
  })
  it('consolida várias propostas do mesmo executivo', () => {
    ...
    expect(ranking).toHaveLength(1); expect(ranking[0].metricas.propostas).toBe(2); expect(ranking[0].metricas.valorProposto).toBe(350); expect(ranking[0].metricas.valorFechado).toBe(220)
  })
})
...
describe('calcularEvolucaoMensal', () => {
  it('agrupa ofertado e vendido pelo mês de criação da proposta', () => {
    const evolucao = calcularEvolucaoMensal([
      linha({ grupo_versao_id: '1', criado_em: '2026-06-10T10:00:00.000Z', valor_total_comercial: 100, negociacao_status: 'fechada', valor_final_negociado: 80 }),
      linha({ grupo_versao_id: '2', criado_em: '2026-06-20T10:00:00.000Z', valor_total_comercial: 200 }),
      linha({ grupo_versao_id: '3', criado_em: '2026-07-05T10:00:00.000Z', valor_total_comercial: 300, negociacao_status: 'fechada', valor_final_negociado: 250 }),
  ...
  it('mantém meses sem propostas para preservar a continuidade da linha', () => {
    ...
    expect(evolucao.map((ponto) => ponto.mes)).toEqual(['2026-05', '2026-06', '2026-07', '2026-08']); expect(evolucao.every((ponto) => ponto.ofertado === 0 && ponto.vendido === 0)).toBe(true)
  })
})
