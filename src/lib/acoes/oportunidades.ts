...
export async function consultarInventarioDaOportunidade(
  programaId: string,
  dataISO: string,
...
export async function publicarOportunidade(
  ...
  if (!programaId || (!proprietario && !sessao.programasVinculados.includes(programaId))) {
    return { ok: false, erro: 'Escolha um programa que você administra.' }
  }
